...
SET @a = CURRENT_TIMESTAMP  
	...
	-- interfering with SELECT statements.
		...
		INSERT	INTO #ServiceHolders (RecurringServiceId, ServiceId, OccurDate, ServiceName, ClientName)
				EXEC [dbo].[GetServiceHoldersTest] @serviceProviderIdContext,
		...
		BEGIN --Set all FieldTypes to their appropriate values
			...
			UPDATE #NumericFields
			SET FieldType = 'Currency' + '-' + FieldName
			WHERE Mask = 'c'

			UPDATE #NumericFields
			SET FieldType = 'Number' + '-' + FieldName
			WHERE Mask = 'g'

			UPDATE #NumericFields
			SET FieldType = 'Percentage' + '-' + FieldName
			WHERE Mask = 'p'

			UPDATE #LocationFields
			SET FieldType = 'Location' + '-' + FieldName

			UPDATE #DateTimeFields
			SET FieldType = 'Time' + '-' + FieldName

			UPDATE #OptionsFields
			SET FieldType = 'ComboBox' + '-' + FieldName
			WHERE TypeInt = 0

			UPDATE #OptionsFields
			SET FieldType = 'CheckBox' + '-' + FieldName
			WHERE TypeInt = 1

			UPDATE #OptionsFields
			SET FieldType = 'CheckList' + '-' + FieldName
			WHERE TypeInt = 2

			UPDATE #TextBoxFields
			SET FieldType = 'TextBox Small' + '-' + FieldName
			WHERE IsMultiLine = 0

			UPDATE #TextBoxFields
			SET FieldType = 'TextBox Large' + '-' + FieldName
			WHERE IsMultiLine = 1
