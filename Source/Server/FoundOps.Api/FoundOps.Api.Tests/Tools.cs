                ...
                foreach (PropertyInfo propertyInfo in objectType.GetProperties(BindingFlags.Public | BindingFlags.Instance).Where(p => p.CanRead && !ignoreList.Contains(p.Name)))
                {
                    ...
                    else if (typeof(IEnumerable).IsAssignableFrom(propertyInfo.PropertyType))
                    {
                        ...
                        else if (valueA != null && valueB != null)
                        {
                            ...
                            {
                                Debug.WriteLine("Collection counts for property '{0}.{1}' do not match.", objectType.FullName, propertyInfo.Name);
                                result = false;
                            }
                            ...
                            else
                            {
                                for (int i = 0; i < collectionItemsCount1; i++)
                                {
                                    ...
                                    if (CanDirectlyCompare(collectionItemType))
                                    {
                                        ...
                                        {
                                            Debug.WriteLine("Item {0} in property collection '{1}.{2}' does not match.", i, objectType.FullName, propertyInfo.Name);
                                            result = false;
                                        }
                                    }
